...
def distribute(partition: Partition, 
    ...
    data_new = {
        k: np.empty(total_to_receive, dtype=data[k].dtype)
    for k in data.keys()}
    ...
    if verify_count:
        local_counts = np.array([len(data[xyz_keys[0]]), len(data_new[xyz_keys[0]])], dtype=np.int64)
        ...
        if rank == 0 and global_counts[0] != global_counts[1]:
            print(f"Error in distribute: particle count during distribute was not maintained ({global_counts[0]} -> {global_counts[1]})", file=sys.stderr, flush=True)
            comm.Abort()
...
def exchange(partition: Partition, 
    ...
    for i in range(exchange_nranks):
        low = orphan_offsets[i]; high = low + orphan_counts[i]
        orphan_ranks[low:high] = i
    exchange_Allgatherv(nonlocal_data, [orphan_data, (orphan_counts, orphan_offsets), nonlocal_data.dtype.char])
    ...
    orphan_requests_send_offsets = np.searchsorted(orphan_requests_send_ranks, np.arange(exchange_nranks))
    orphan_requests_send_counts = np.append(orphan_requests_send_offsets[1:], len(orphan_requests_send)) - orphan_requests_send_offsets
    ...
    orphan_requests_recv_offsets = np.insert(np.cumsum(orphan_requests_recv_counts)[:-1], 0, 0)
    orphan_requests_recv = np.empty(orphan_requests_recv_total, dtype=orphan_requests_send.dtype)
    exchange_Alltoallv(
        [orphan_requests_send, (orphan_requests_send_counts, orphan_requests_send_offsets), orphan_requests_send.dtype.char],
        [orphan_requests_recv, (orphan_requests_recv_counts, orphan_requests_recv_offsets), orphan_requests_recv.dtype.char]
    )
    ...
    if do_all2all and (orphan_requests_send_counts[rank] != 0 or orphan_requests_recv_counts[rank] != 0):
        print(f"Error in exchange: rank {rank} is asking itself for an orphan halo: {orphan_requests_send_counts[rank]}/{orphan_requests_recv_counts[rank]}", file=sys.stderr, flush=True)
        comm.Abort()
    ...
    for i in range(exchange_nranks):
        req = orphan_requests_recv[orphan_requests_recv_offsets[i]:orphan_requests_recv_offsets[i]+orphan_requests_recv_counts[i]]
        mask = np.isin(data[key], req)
        orphan_requests_indices.append(np.nonzero(mask)[0])
        orphan_requests_mask |= mask
    orphan_requests_send_counts = np.array([len(i) for i in orphan_requests_indices], dtype=np.int32)
    orphan_requests_recv_counts = np.empty_like(orphan_requests_send_counts)
    exchange_Alltoall(orphan_requests_send_counts, orphan_requests_recv_counts)
    orphan_requests_recv_total = np.sum(orphan_requests_recv_counts)
    orphan_requests_send_offsets = np.insert(np.cumsum(orphan_requests_send_counts)[:-1], 0, 0)
    orphan_requests_recv_offsets = np.insert(np.cumsum(orphan_requests_recv_counts)[:-1], 0, 0)
    ...
    for k in data.keys():
        ...
        data_new[k] = np.concatenate((data[k][~orphan_requests_mask], orphan_requests_recv))
    ...
    localcounts = np.array([localcount, localcount_after, localcount_missmatch, localcount_missmatch_after], dtype=np.int64)
    totalcounts = np.empty_like(localcounts)
    comm.Allreduce(localcounts, totalcounts)
    totalcount_before, totalcount_after, totalcount_missmatch, totalcount_missmatch_after = totalcounts

    if verbose and rank == 0:
        print(f"exchange summary ({'all2all' if do_all2all else '26-neighbors'}):")
        print(f"   Ntot -> Ntot: {totalcount_before:10d} -> {totalcount_after:10d} (should remain the same)")
        print(f"   Orph -> Orph: {totalcount_missmatch:10d} -> {totalcount_missmatch_after:10d} (should be 0 after)")
        print("", flush=True)

    # did we conserve number of particles?
    if rank == 0 and totalcount_before != totalcount_after:
        print(f"Error in exchange: Lost halos during progenitor exchange: {totalcount_before} -> {totalcount_after}", file=sys.stderr, flush=True)
        comm.Abort()
    ...
    if not do_all2all and totalcount_missmatch_after > 0:
        if verbose and rank == 0:
            print(f"exchange all2all since neighbor exchange was not able to assign all: {totalcount_missmatch} -> {totalcount_missmatch_after}", flush=True)
        return exchange(partition, data_new, key, local_keys, 
            verbose=verbose, 
    ...
    for i in range(nranks):
        if rank == i:
            if localcount_missmatch_after != 0:
                print(f"Warning from rank {rank} in exchange: Unable to assign all progenitors to correct ranks (failed for {localcount_missmatch_after} out of {localcount_missmatch})")
    ...
    if rank == 0 and totalcount_missmatch_after != 0:
        if replace_notfound_key is None:
            print(f"Error in exchange: Unable to assign all progenitors to correct ranks (tried to reassign {totalcount_missmatch}, failed for {totalcount_missmatch_after})", file=sys.stderr, flush=True)
            comm.Abort()
        else:
            print(f"Warning in exchange: Unable to assign all progenitors to correct ranks (tried to reassign {totalcount_missmatch}, failed for {totalcount_missmatch_after}), replacing missing values with {replace_notfound_key}", flush=True)
