...
def merger_tree_drawing(
    ...
    if methods[0] == 'recursive':
        hv, hy = _tree_recursive(hroot, hlist, hloc, snaplists, padding, align=methods[1])
    else:
    ...
    if highlight_mm:
        ...
        def highlight_branches(h):
            ...
            if len(h.progenitors):
                ...
                abs_threshold = mm_threshold if mm_is_absolute else mm_threshold * h.progenitors[0].mass
    ...
    for i in range(nhalos):
        d.append(drawSvg.Rectangle(hx[i], hy[i], hh[i], hv[i], fill=colors[i], fill_opacity=1 if highlights[i] else 0.3))
    ...
    for ld, lp in links:
        ...
        d.append(drawSvg.Path(stroke='none', fill=colors[lp], fill_opacity=0.6 if highlights[lp] else 0.1).M(_x0, _y0).v(_h).c(_s0, 0, _s1, _dy, _dx, _dy).v(-_h).c(-_s0, 0, -_s1, -_dy, -_dx, -_dy))
