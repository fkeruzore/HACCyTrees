import numpy as np
import numba
from typing import Tuple, Mapping, List, overload


@overload
def split_fragment_tag(tag: int) -> Tuple[int, int]: ...
@overload
def split_fragment_tag(tag: np.ndarray) -> Tuple[np.ndarray, np.ndarray]: ...


def split_fragment_tag(tag):
        ...
        if snapnum[i] >= lastsnap:
            ...
            if start_index < i - 1:
                ...
                for j in range(1, nsteps):
                    data_out[start_index + j] = (valstart + j * dval * (not masked)) + (
                        dval_mask * masked
                    )
            data_out[i] = data_in[i]
            start_index = i
    ...
    *,
    ...
    for k in keys:
        ...
        if interpolation == "constant_reverse":
            _fix_fragment_property_constrev(
                forest["snapnum"], forest["fof_halo_tag"], data, data_out
            )
        elif interpolation == "linear":
            _fix_fragment_property_linear(
                forest["snapnum"], forest["fof_halo_tag"], data, data_out, mask_negative
            )
        else:
